...
The Boost Graph Library is widely used but lacks community detection methods such as Louvain [@Blondel2008], Leiden [@Traag2019], or the Stochastic Block Model [@Holland1983]. Existing Louvain implementations like igraph (C) [@Csardi2006], NetworkX (Python) [@Hagberg2008], or gen-louvain (C++) [@Campigotto2013software,@Campigotto2014] hardcode their graph data structures into the algorithm, requiring users to convert their data before use. This calls for a generic implementation built on the abstract data access patterns that BGL graph concepts already provide.
